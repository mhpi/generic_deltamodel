...
class prms_marrmot_refreeze(torch.nn.Module):
    def __init__(self):
        super(prms_marrmot_refreeze, self).__init__()
        ...
        self.parameters_bound = dict(tt=[-2.5, 2.5], #[-3, 5],    # tt, Temperature threshold for snowfall and melt [oC]
                                     ...
                                     retip=[0, 50], #[0, 50],    # retip, Maximum impervious area storage [mm]
                                     ...
                                     stot=[1, 1000],   # [1, 2000] # stot, Total soil moisture storage [mm]: REMX+SMAX
                                     ...
                                     k2=[1.5, 4.0], #[1, 5],    # k2, Groundwater drainage non-linearity [-]
                                     k3=[0, 1],    # k3, Interflow coefficient 1 [d-1]
                                     # k4=[0, 1],   # k4, Interflow coefficient 2 [mm-1 d-1]
                                     k5=[0, 0.5], #[0, 1],    # k5, Baseflow coefficient [d-1]
    ...
    def forward(self, x_hydro_model, c_hydro_model, params_raw, args,  warm_up=0, init=False, routing=True, conv_params_hydro=None):
        ...
        if warm_up > 0:
            with torch.no_grad():
                ...
                warm_up_model = prms_marrmot_refreeze().to(args["device"])
                Q_init, snow_storage, XIN_storage, RSTOR_storage, \
                    RECHR_storage, SMAV_storage, \
                    RES_storage, GW_storage, MELTWATER_storage = warm_up_model(xinit, c_hydro_model, params_raw, args,
            ...
            MELTWATER_storage = torch.zeros([x_hydro_model.shape[1], nmul], dtype=torch.float32,
                                       device=args["device"]) + 0.001
        ...
        for t in range(Ndays):
            ...
            melt = params_dict["ddf"] * (T - params_dict["tt"])
            melt = torch.clamp(melt, min=0.0)
            melt = torch.min(melt, snow_storage)
            MELTWATER_storage = MELTWATER_storage + melt
            snow_storage = snow_storage - melt
            refreezing = params_dict["parCFR"] * params_dict["ddf"] * (
                    params_dict["tt"] - T)
            refreezing = torch.clamp(refreezing, min=0.0)
            refreezing = torch.min(refreezing, MELTWATER_storage)
            snow_storage = snow_storage + refreezing
            MELTWATER_storage = MELTWATER_storage - refreezing
            flux_m = MELTWATER_storage - (params_dict["parCWH"] * snow_storage)
            flux_m = torch.clamp(flux_m, min=0.0)
            MELTWATER_storage = MELTWATER_storage - flux_m
            ...
            # Refreeze_RSTOR = Refreez_flux * (1 - params_dict["beta"])
            # Refreeze_RSTOR = torch.min(RSTOR_storage, Refreeze_RSTOR)
            # RSTOR_storage = torch.clamp(RSTOR_storage - Refreeze_RSTOR, min=NEARZERO)
            # snow_storage = snow_storage + Refreeze_RSTOR
            ...
            flux_pc = RECHR_storage - remx   # remx is maximum RECH_storage
            ...
            # Refreeze_RECHR = Refreez_flux * params_dict["beta"]
            # Refreeze_RECHR = torch.min(RECHR_storage, Refreeze_RECHR)
            # RECHR_storage = torch.clamp(RECHR_storage - Refreeze_RECHR, min=NEARZERO)
            # snow_storage = snow_storage + Refreeze_RECHR
        ...
        if init:  # means we are in warm up. here we just return the storages to be used as initial values
            return Qsrout, snow_storage, XIN_storage, RSTOR_storage, \
                RECHR_storage, SMAV_storage, RES_storage, GW_storage, MELTWATER_storage
        else:
