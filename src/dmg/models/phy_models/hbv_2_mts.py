...
class Hbv_2_mts(torch.nn.Module):
    ...
    def __init__(
        ...
        self.state_transfer_model = torch.nn.ModuleDict({
            name: torch.nn.Sequential(torch.nn.Linear(self.low_freq_model.nmul, self.high_freq_model.nmul),
                                      torch.nn.ReLU())
        ...
        self.simulate_spatial_chunk_size = high_freq_config['simulate_spatial_chunk_size']
        self.simulate_temporal_chunk_size = high_freq_config['simulate_temporal_chunk_size']
        self.spatial_chunk_size = self.train_spatial_chunk_size
        self.simulate_mode = False

        self.train_warmup = high_freq_config['train_warmup']  # warmup steps for routing during training

    def _forward(self,
                 x_dict: dict[str, torch.Tensor],
                 parameters: tuple[list[torch.Tensor], list[torch.Tensor]]) -> dict[str, torch.Tensor]:
        """Base forward"""
        ...
        # transfer parameters
        phy_dy_params_dict, phy_static_params_dict, distr_params_dict = self.param_transfer(
            low_freq_parameters,
        ...
        Elevation = x_dict['elev_all'].unsqueeze(-1).expand(-1, self.high_freq_model.nmul)
        outlet_topo = x_dict['outlet_topo']
        areas = x_dict['areas']

        predictions = self.high_freq_model.PBM(forcing=x,
                                               Ac=Ac,
                                               Elevation=Elevation,
                                               states=tuple(states),
    ...
    def forward(self,
        ...
        self.high_freq_model.use_distr_routing = False
        preds_list = []
        for i in tqdm(range(0, n_units, spatial_chunk_size), desc="Spatial runoff chunks"):
            # print('i: ', i)
            ...
            chunk_parameters = (
                [
                    parameters[0][0][:, i:end_idx].to(device),  # low-freq dynamic phy params
                    parameters[0][1][i:end_idx].to(device),  # low-freq static phy params
                ],
                [
                    parameters[1][0][:, i:end_idx].to(device),  # high-freq dynamic phy params
                    parameters[1][1][i:end_idx].to(device),  # high-freq static phy params
                    parameters[1][2][idxs_in_chunk].to(device),  # high-freq distributed params
                ]
            )
        ...
        distr_params_dict = {key: value.to(device) for key, value in distr_params_dict.items()}
        outlet_topo = x_dict['outlet_topo'].to(device)
        areas = x_dict['areas'].to(device)
        total_steps = (high_freq_length - train_warmup + temporal_chunk_size - 1) // temporal_chunk_size
        preds_list = []
        for t in tqdm(range(train_warmup, high_freq_length, temporal_chunk_size), desc="Temporal routing chunks"):
            # print('t: ', t)
            ...
            if t > train_warmup:  # remove routing warmup for all but first chunk
                chunk_predictions = {key: value[train_warmup:] for key, value in chunk_predictions.items()}
            preds_list.append(chunk_predictions)
    ...
    def concat_spatial_chunks(self, pred_list: List[Dict[str, torch.Tensor]]):
        ...
        for key in pred_list[0].keys():
            if pred_list[0][key].ndim == 3:
                output[key] = torch.cat([preds[key] for preds in pred_list], dim=1)  # (window_size, n_units, nmul)
            else:
                output[key] = torch.cat([preds[key] for preds in pred_list], dim=0)  # (n_units, nmul) or (n_units,)
        return output
    ...
    def concat_temporal_chunks(self, pred_list: List[Dict[str, torch.Tensor]]):
        ...
        for key in pred_list[0].keys():
            if pred_list[0][key].ndim == 3:
                output[key] = torch.cat([preds[key] for preds in pred_list], dim=0)  # (window_size, n, nmul)
            else:
    ...
    def param_transfer(self,
        ...
        warmup_phy_dy_params, warmup_phy_static_params, warmup_routing_params = self.low_freq_model.unpack_parameters(
            low_freq_parameters)
        phy_dy_params, phy_static_params, routing_params, distr_params = self.high_freq_model.unpack_parameters(
            high_freq_parameters)
        # new dynamic params
        phy_dy_params_dict = self.high_freq_model.descale_phy_dy_parameters(phy_dy_params,
                                                                            dy_list=self.high_freq_model.dynamic_params)
        # keep warmup static params, add high-freq specific static params
        static_param_names = [param for param in self.high_freq_model.phy_param_names if
                              param not in self.high_freq_model.dynamic_params]
        warmup_static_param_names = [param for param in self.low_freq_model.phy_param_names if
                                     param not in self.low_freq_model.dynamic_params]
        var_indexes = [i for i, param in enumerate(static_param_names) if param not in warmup_static_param_names]
        phy_static_params_dict = self.high_freq_model.descale_phy_stat_parameters(
            torch.concat([warmup_phy_static_params, phy_static_params[:, var_indexes]], dim=1),
            stat_list=static_param_names)
        # new distributed params
        distr_params_dict = self.high_freq_model.descale_distr_parameters(distr_params)
        # new routing params
        if self.high_freq_model.routing:
            self.high_freq_model.routing_param_dict = self.high_freq_model.descale_rout_parameters(routing_params)
    ...
    def state_transfer(self, states: list[torch.Tensor]):
        ...
        return [self.state_transfer_model[key](states_dict[key]) for key in states_dict.keys()]
