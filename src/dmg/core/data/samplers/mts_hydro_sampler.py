...
from distributedDS.core.data.loaders.mts_hydro_loader import DistributedDataSchema
...
class MtsHydroSampler(BaseSampler):
    ...
    def get_samples(self, dataset: DistributedDataSchema, mode: str = 'train'):
        ...
        """
        config = self.config
        window_size = config['delta_model']['phy_model']['high_freq_model']['window_size']
        stride = config['train']['stride']
        warmup_daily = config['delta_model']['phy_model']['low_freq_model']['window_size']
        warmup_hourly = config['delta_model']['phy_model']['high_freq_model']['train_warmup']
        agg_timescale_dim = tuple(config['delta_model']['phy_model']['high_freq_model']['agg_timescale_dim'])
        ...
        self.time_window_loader = TimeWindowLoader(data=dataset, k=k, stride=stride, warmup_daily=warmup_daily,
                                                   warmup_hourly=warmup_hourly, agg_timescale_dim=agg_timescale_dim,
                                                   batch_size=batch_size, shuffle=shuffle, is_seamless=is_seamless)
...
def get_gauge_subset(dataset: DistributedDataSchema, gauges: List[str]) -> DistributedDataSchema:
    ...
    subset = DistributedDataSchema(
        ...
        rout_static_input=dataset.rout_static_input[gauge_indexes, :, :][:, unit_indexes, :],
    ...
    if dataset.scaled_target is not None:
        ...
        subset.scaled_rout_static_input = dataset.scaled_rout_static_input[gauge_indexes, :, :][:, unit_indexes, :]
    return subset


def get_time_subset(dataset: DistributedDataSchema, t0: int, t1: int, warmup_daily: int = 365, warmup_hourly: int = 24 * 7) -> DistributedDataSchema:
...
class TimeWindowBatchSampler(Sampler):
    ...
    def __init__(self, batch_size: int, Y: torch.Tensor, stride: int, k: int, warmup_daily: int = 365, warmup_hourly: int = 24 * 7, is_seamless: bool = False, shuffle=True):
        super().__init__(None)
        self.windows_by_t = self._build_windows_by_t(Y, stride=stride, k=k, warmup_daily=warmup_daily, warmup_hourly=warmup_hourly, is_seamless=is_seamless)
    ...
    def __len__(self):
        return sum((len(v) + self.batch_size - 1) // self.batch_size for v in self.windows_by_t.values())

    @staticmethod
    def _build_windows_by_t(Y: torch.Tensor, stride: int, k: int, warmup_daily: int = 365, warmup_hourly: int = 24 * 7, is_seamless: bool = False) -> dict[int, list[int]]:
        n_s, n_t = Y.shape
        ...
        for t in range(t_first, t_last_inclusive + 1, stride):
            if is_seamless:
                ...
                idxs = [i for i in range(n_s) if mask[i, t + warmup_hourly: t + k].any()]
...
class TimeWindowLoader:
    ...
    def __init__(self, data: DistributedDataSchema, k: int, stride: int, warmup_daily: int, warmup_hourly: int,
                 agg_timescale_dim: Tuple, batch_size: int, shuffle: bool = False, is_seamless: bool = False):
        """
        :param data: dataset
        ...
        self.sampler = TimeWindowBatchSampler(batch_size=batch_size, Y=data.target, stride=stride, k=k,
                                              warmup_daily=warmup_daily, warmup_hourly=warmup_hourly,
                                              is_seamless=is_seamless, shuffle=shuffle)
    ...
    def _build_batch(self, t: int, sids: list[int]) -> DistributedPhySampleSchema:
        ...
        global_gauge_indexes = torch.tensor([data.gauge_index[i] for i in local_gauge_indexes])
        unit_indexes = torch.nonzero(data.topo[local_gauge_indexes, :].sum(axis=0)).squeeze(-1)
        ...
        x_phy_daily = x_phy_daily.contiguous().view(x_phy_daily.shape[0], -1, 24, x_phy_daily.shape[-1]).mean(dim=2)
        ...
        xc_nn_norm_daily = xc_nn_norm_daily.contiguous().view(xc_nn_norm_daily.shape[0], -1, 24, xc_nn_norm_daily.shape[-1]).mean(dim=2)
        xc_nn_norm_daily = xc_nn_norm_daily.swapaxes(0, 1).contiguous()
        xc_nn_norm_daily = torch.concat([xc_nn_norm_daily, c_nn_norm.unsqueeze(0).repeat(xc_nn_norm_daily.shape[0], 1, 1)], dim=2)
        ...
        xc_nn_norm_hourly = torch.concat([xc_nn_norm_hourly, c_nn_norm.unsqueeze(0).repeat(xc_nn_norm_hourly.shape[0], 1, 1)], dim=2)
        ...
        rout_c_nn = data.scaled_rout_static_input[local_gauge_indexes, :, :][:, unit_indexes, :]
        rout_c_nn = rout_c_nn.reshape(-1, rout_c_nn.shape[-1])[outlet_topo.flatten().bool()]
