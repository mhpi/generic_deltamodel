...
def main(config: DictConfig) -> None:
    try:
        ...
        ## Save Path ##
        # save_path = '/data/lgl5139/project_blue_eyes/generic_diffModel/results/camels_531/train_1999_2008/3_forcing/mosaic_argmax/' + stat_name + '/HBV_1_1p_PRMS_SACSMA_with_snow_/NSE_loss_batch'
        # save_path = '/data/lgl5139/project_blue_eyes/generic_diffModel/results/camels_531/train_1999_2008/3_forcing/mosaic_argmax/' + stat_name + '/PRMS_SACSMA_with_snow_/NSE_loss_batch'
        # save_path = '/data/lgl5139/project_blue_eyes/generic_diffModel/results/camels_531/train_1999_2008/3_forcing/mosaic_argmax/' + stat_name + '/HBV_1_1p_SACSMA_with_snow_/NSE_loss_batch'
        save_path = '/data/lgl5139/project_blue_eyes/generic_diffModel/results/camels_531/train_1999_2008/3_forcing/mosaic_argmax/' + stat_name + '/HBV_1_1p_PRMS_/NSE_loss_batch'
        ...
        train_metric = torch.zeros(train_preds.shape[0], train_preds.shape[2])  # [num_models, n_basins]
        for i, preds in enumerate(train_preds):
            train_metric[i, :] = torch.from_numpy(metrics(
                np.swapaxes(preds.numpy(), 1, 0),
                np.swapaxes(train_obs.numpy(), 1, 0)
            )[stat_name])

        # Identify the best model for each basin
        best_model_indices = torch.argmax(train_metric, dim=0)  # [n_basins]

        ### Generate Mosaic ###
        log.info("Creating mosaic predictions...")
        mosaic_preds = torch.zeros(test_preds.shape[1], test_preds.shape[2])  # [n_test_samples, n_basins]

        for basin_idx in range(test_preds.shape[2]):
            best_model = best_model_indices[basin_idx]
            mosaic_preds[:, basin_idx] = test_preds[best_model, :, basin_idx]

        ### Evaluate Metrics ###
        log.info("Evaluating metrics on test predictions...")
        test_obs = get_dataset_dict(config, train=False)['target'][warm_up:, :, 0]  # [n_test_samples, n_basins]
        metrics_dict = metrics(
            np.swapaxes(mosaic_preds.numpy(), 1, 0),
        ...
        np.save(os.path.join(save_path, "mosaic_preds.npy"), mosaic_preds.numpy())
        np.save(os.path.join(save_path, "test_obs.npy"), test_obs)
        np.save(os.path.join(save_path, "best_model_indices.npy"), best_model_indices.numpy())
        ...
        log.info(f"Time Elapsed: {(total_time / 60):.2f} minutes")
