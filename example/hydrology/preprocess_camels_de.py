...
def _read_catchment_series(de_root: pathlib.Path, cid: str) -> pd.DataFrame:
    """Return a per-catchment DataFrame on the ALL_TIME index with P, T, PET, Q.

    Parameters
    ----------
    de_root
        Root directory of CAMELS-DE v1.0.0 containing the timeseries/ and
        timeseries_simulated/ CSVs.
    cid
        Catchment ID string matching the CAMELS-DE CSV naming convention and the
        gauge_id in the attribute CSVs.

    Returns
    -------
    pd.DataFrame
        DataFrame indexed by date with columns ['P', 'T', 'PET', 'Q'].
    """
    ts = (
        pd.read_csv(
    ...
    sim = (
        ...
        .rename(columns={'pet_hargreaves': 'PET'})
        .set_index('date')
    )
...
def build_forcings_target(
    de_root: pathlib.Path,
    ids: list,
) -> tuple[np.ndarray, np.ndarray]:
    """Return (forcings, target) arrays for the given catchment IDs.
    ...
    de_root
        ...
        timeseries_simulated/ CSVs.
    ids
        List of catchment IDs to include, matching the gauge_id in the attribute
        CSVs and the {id} in the timeseries CSV filenames.

    Returns
    -------
    tuple[np.ndarray, np.ndarray]
        Tuple of (forcings, target) arrays.
    """
...
def build_attributes(
    ...
    """Return attributes array of shape (N, A) matching ATTR_SPEC.

    Parameters
    ----------
    de_root
        Root directory of CAMELS-DE v1.0.0 containing the attribute CSVs.
    ids
        List of catchment IDs to include, matching the gauge_id in the attribute
        CSVs.
    forcings
        The (N, T, 3) forcings array built by build_forcings_target, used to
        compute pet_mean and aridity.

    Returns
    -------
    np.ndarray
        Array of shape (N, A) with attributes in the order specified by ATTR_SPEC.
    """
    clim = pd.read_csv(de_root / "CAMELS_DE_climatic_attributes.csv").set_index(
        'gauge_id'
    )
    topo = pd.read_csv(de_root / "CAMELS_DE_topographic_attributes.csv").set_index(
        'gauge_id'
    )
    soil = pd.read_csv(de_root / "CAMELS_DE_soil_attributes.csv").set_index('gauge_id')
    land = pd.read_csv(de_root / "CAMELS_DE_landcover_attributes.csv").set_index(
        'gauge_id'
    )
    ...
    if nan_per_col.any():
        print(
            f"  NaNs per column before fill: {dict(zip(ATTR_SPEC, nan_per_col.tolist()))}"
        )
...
def main():
    """Preprocess CAMELS-DE into a pickle for HydroLoader."""
